...
<html>
	...
	<body>
		<?php
			...
			$varpdatePost 	= isset($_POST['supproddate2']) ? $_POST['supproddate2'] : "";
			$varlotnoPost 	= isset($_POST['lotnosupp2']) ? $_POST['lotnosupp2'] : "";
			
			$varuniquecode = ""; $varunique = ""; $varpdate = ""; $varlotno = "";
			$txtso = ""; $varpart = ""; $varpo = ""; $varqty = ""; 
			if($varpartPost1 != "" and $varpartPost2 == ""){
				$totchar 		= strlen($varpartPost1);
				$checkdate      = strlen(trim(substr($varpartPost1,77,8)));
				
				if ($totchar <= 86 and $checkdate == 0) {
				...
				else if($totchar == 86 and $checkdate != 0) {
					$varpartPost	= strtoupper($varpartPost1);
					$txtnik	    	= strtoupper(trim($txtnikPost1));
					$txtso	    	= strtoupper(trim($txtsoPost1));
					$varpart    	= trim(substr($varpartPost,0,15));
					...
					$varunique  	= substr($varpartPost,30,46); //( 1+6+15+18+6 = 46 ) //( 15+8+6 = 29 )
					$varuniquecode	= substr($varpartPost,56,20); //( 1 +6 +15+18+6 = 46 ) (57+20 = month+day+time+microtime+code) //( 49+10 = month+day+code )
					$varpdate    	= substr($varpartPost,77,8); //proddate
					$varlotno 		= "";
				}
				else if($totchar > 86 and $checkdate == 0) {
					$varpartPost	= strtoupper($varpartPost1);
					...
					$varunique  	= substr($varpartPost,30,46); //( 1+6+15+18+6 = 46 ) //( 15+8+6 = 29 )
					$varuniquecode	= substr($varpartPost,56,20); //( 1 +6 +15+18+6 = 46 ) (57+20 = month+day+time+microtime+code) //( 49+10 = month+day+code )
					$varpdate    	= ""; //supproddate
					$varlotno 		= substr($varpartPost,86,20); //lotnosup
				}
				else if($totchar > 86 and $checkdate != 0) {
					$varpartPost	= strtoupper($varpartPost1);
					$txtnik	    	= strtoupper(trim($txtnikPost1));
				...
				$varqty     = strtoupper(trim($varqtyPost));
				$varqty     = strtoupper(trim($varqtyPost));
				$varpdate   = strtoupper(trim($varpdatePost));
				$varlotno   = strtoupper(trim($varlotnoPost));
			}
						...
						if ($cekpart == 1)
						{
							...
							if ($checkunix == 46 or $varunique == "" or $checkunix == 0)
							{
								if($duplikat == 0)
								{
									...
									//	part OK
											$sqlins = "insert into partiss(so,partno,partname,po,bom,reqqty,scanqty,lot,line,model,issdate, nik, proddate, lotnosup) 
													values('$txtso','$varpart','$nilainama','$varpo','$nilaibom','$nilaireq','$varqty','$nilailot',
													'$nilailine','$nilaimodel','$tglins','$txtnik','$varunique','$varpdate','$varlotno')";
								...
								else
								{
									...
									echo '<audio controls autoplay hidden="hidden"><source src="sound/partsudahdigunakan.mp3" type="audio/mp3"></audio>';
								}
							}
							else
							{
