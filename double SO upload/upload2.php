...
echo '<a href="index.php">menu</a><br />Data hasil upload : <br />';
  ...
  { 		
     ...
     //   $kopifile = "d:\\xampp\\tmp\\so.csv";            
      //  copy($namatemp,  $kopifile);      26 Agustus 2015		//	echo 'set quoted';
        ...
        while ($i = fgetcsv($feed, 10000, ",")) 
		{
			...
			$rsinsert = $db->Execute("select top 1 * from sa90temp_so");
	...
	// hapus data di sa90
        $sqldelsa = "delete from sa90_so";
        $rsdelsa = $db->Execute($sqldelsa);
		$rsdelsa->Close();			
        // insert data ke sa90 dari sa90temp
        $sqlins = "insert into sa90_so select convert(char,c),convert(char,i),j,convert(float,p),convert(float,q),ad,an,ac,convert(char,ab) from sa90temp";	       
		$hasilins= $db->Execute($sqlins);
		$hasilins->Close();
		
 	// bersihkan double quote
	// echo 'Bersihkan double quote<br />';			
        $sqlupd = "update sa90_so set so = REPLACE(so,CHAR(34),''), partno = REPLACE(partno,CHAR(34),'')";
        ...
        while(!$brsbaca->EOF)
          {
			...
			// cek so di sa90m
			$sqlada_so = "select so, partno, partname, bom, qty, lot, line, model, replikasi from sa90m_so where so = '$cekso'"; //by zaki 20170421
            $rshasil_so = $db->Execute("select so, partno, partname, bom, qty, lot, line, model, replikasi from sa90m_so where so = '$cekso'"); //by zaki 20170421
	        $cekdata_so = $rshasil_so->RecordCount(); //by zaki 20170421
	        $cekmodel_so = trim($rshasil_so->fields[7]); //by zaki 20170421
			
			if($cekdata_so != 0 && $cekmodel_so != $cekmodel2){ //by zaki 20170421
				$sqlins_dup = " insert into sa90m_dup select so,partno,partname,bom,qty,lot,line,model from sa90_so where so = '$cekso' and partno = '$cekpart'";
				$rsins_dup= $db->Execute($sqlins_dup);
				
				$sqldel_so 	= "delete from sa90m_so where so = '$cekso' and model = '$cekmodel_so'"; //by zaki 20170421
				$rsdel_so	= $db->Execute($sqlins); //by zaki 20170421
				
				$sqlada = "select * from sa90m_so where so = '$cekso' and partno = '$cekpart'";
				$rshasil = $db->Execute("select * from sa90m_so where so = '$cekso' and partno = '$cekpart'");
				...
				if($cekdata == 0){
						...
						echo ' is nothing.. inserting ...<br />';
							$sqlins = " insert into sa90m_so select so,partno,partname,bom,qty,lot,line,model from sa90 where so = '$cekso' and partno = '$cekpart'";
						...
						$sqlup = "update sa90m_so set partname = '$cekpartname', bom = $cekbom, qty = $cekqty, lot = '$ceklot', ";
				...
				if($cekdata == 0){
						...
						echo $cekpart;
						echo ' is nothing.. inserting ...<br />';
							$sqlins = " insert into sa90m_so select so,partno,partname,bom,qty,lot,line,model from sa90_so where so = '$cekso' and partno = '$cekpart'";
							$rsins= $db->Execute($sqlins);
				}             
				else{
						$baris++;  		  
...
$del_sa90prodtemp = "delete from sa90prodtemp_so";
$rsdpt	= $db->Execute($del_sa90prodtemp);
$rsdpt->close();

// insert data ke sa90prodtemp dari sa90temp			
$ins_sa90prodtemp = "insert into sa90prodtemp_so select convert(char,c),convert(char,i),j,convert(float,p),convert(float,q),ad,an,ac,aa from sa90temp_so";//$rsipt = $db->Execute($ins_sa90prodtemp);
//$rsipt->close();
// bersihkan double quote			$upd_sa90prodtemp = "update sa90prodtemp_so set so = REPLACE(so,CHAR(34),''), partno = REPLACE(partno,CHAR(34),'')";			$upd_sa90prodtemp .= " , partname = REPLACE(partname,CHAR(34),''), lot = REPLACE(lot,CHAR(34),'')";
...
$barisprod = 0;
$sel_sa90prodtemp	= "select * from sa90prodtemp_so";$row = $db->Execute($sel_sa90prodtemp);			
...
while(!$row->EOF)
	...
	// cek data di table SA90M --> jika belum ada --> do insert , jika sudah ada --> do update
	$sel_sa90prod 	= "select * from sa90prod_so where so = '$so' and partno = '$part'";
	...
	if($cek == 0)          
	  {
		...
		echo $part;
		echo ' is nothing.. inserting ...<br />';
				$ins_sa90prod = "insert into sa90prod_so select * from sa90prodtemp where so = '$so' and partno = '$part'";					
		...
		echo ' data already exist, updating ...<br />';
			...
			$upd_sa90prod = "update sa90prod_so set partname = '$partname', bom = $bom, qty = $qty, lot = '$lot', ";
