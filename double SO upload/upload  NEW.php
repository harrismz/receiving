        ...
        while(!$brsbaca->EOF)
          {
			...
			// cek so di sa90m
			//echo $baris.'--'.$sqlada_so = "select distinct so, model from sa90m_so1 where so = '$cekso'"; //by zaki 20170421
            //$rshasil_so = $db->Execute("select distinct so, model from sa90m_so1 where so = '$cekso'"); //by zaki 20170421
			
			$sqlada_so = "select top 1 so, model, max(replikasi) as replikasi from sa90m_so1
									where so = '$cekso'
									group by so,model
									order by replikasi asc"; //by zaki 20170421
            $rshasil_so = $db->Execute("select top 1 so, model, max(replikasi) as replikasi from sa90m_so1
									where so = '$cekso'
									group by so,model
									order by replikasi asc"); //by zaki 20170421
			...
			$cekmodel_so = trim($rshasil_so->fields[1]); //by zaki 20170421
		 	...
		 	$sqlada = "select * from sa90m_so1 where so = '$cekso' and partno = '$cekpart'";
			$rshasil = $db->Execute("select * from sa90m_so1 where so = '$cekso' and partno = '$cekpart'");
			$cekdata = $rshasil->RecordCount();	  
			
//			if($cekdata == 0){
//				echo '<br><br>1 if ['.$cekso.' - '.$cekpart.' - '.$cekmodel2.'] checkdata so = '.$cekdata_so.';  --  modelso = '.$cekmodel_so.'; -- cekmodel = '.$cekmodel2.';<br>';				
//				echo '------------------------------------------------------------------------------------------------------------------------------------------------<br>';				
//				
//					$baris++;
//								  
...
//						$sqlins = " insert into sa90m_so1 select so,partno,partname,bom,qty,lot,line,model from sa90_so1 where so = '$cekso' and partno = '$cekpart'";
//						$rsins= $db->Execute($sqlins);
//			}             
//			else{
//				echo '<br><br>2 else ['.$cekso.' - '.$cekpart.' - '.$cekmodel2.'] checkdata so = '.$cekdata_so.';  --  modelso = '.$cekmodel_so.'; -- cekmodel = '.$cekmodel2.';<br>';				
//				echo '------------------------------------------------------------------------------------------------------------------------------------------------<br>';				
//				
//					$baris++;  		  
//					echo '** ';
...
//					$sqlup = "update sa90m_so1 set partname = '$cekpartname', bom = $cekbom, qty = $cekqty, lot = '$ceklot', ";
...
//			}
			...
			if($cekdata_so == 0 or empty($cekdata_so)){
				...
				if($cekmodel_so == $cekmodel2){
					//echo '<br><br>2 else if ['.$cekso.' - '.$cekpart.' - '.$cekmodel2.'] checkdata so = '.$cekdata_so.';  --  modelso = '.$cekmodel_so.'; -- cekmodel = '.$cekmodel2.';<br>';				
					//echo '------------------------------------------------------------------------------------------------------------------------------------------------<br>';				
					if($cekdata == 0){
							$baris++;
				...
				elseif($cekmodel_so != $cekmodel2){
					//echo '<br><br>3 else elseif ['.$cekso.' - '.$cekpart.' - '.$cekmodel2.'] checkdata so = '.$cekdata_so.';  --  modelso = '.$cekmodel_so.'; -- cekmodel = '.$cekmodel2.';<br>';				
					//echo '------------------------------------------------------------------------------------------------------------------------------------------------<br>';				
					$sqlins_dup = "insert into sa90m_dup1 select so,partno,partname,bom,qty,lot,line,model,CONVERT(varchar(20),getdate(),120) from sa90_so1 where so = '$cekso' and partno = '$cekpart'";
					$rsins_dup= $db->Execute($sqlins_dup);
					
					$sqldel_so 	= "delete from sa90m_so1 where so = '$cekso' and model = '$cekmodel_so'"; //by zaki 20170421
...
$del_sa90prodtemp = "delete from sa90prodtemp_so1";
$rsdpt	= $db->Execute($del_sa90prodtemp);
$rsdpt->close();

// insert data ke sa90prodtemp dari sa90temp
			
$ins_sa90prodtemp = "insert into sa90prodtemp_so1 select convert(char,c),convert(char,i),j,convert(float,p),convert(float,q),ad,an,ac,aa from sa90temp_so1";
//$rsipt = $db->Execute($ins_sa90prodtemp);
//$rsipt->close();
// bersihkan double quote
			$upd_sa90prodtemp = "update sa90prodtemp_so1 set so = REPLACE(so,CHAR(34),''), partno = REPLACE(partno,CHAR(34),'')";
...
$barisprod = 0;
$sel_sa90prodtemp	= "select * from sa90prodtemp_so1";
...
while(!$row->EOF)
	...
	// cek data di table SA90M --> jika belum ada --> do insert , jika sudah ada --> do update
	$sel_sa90prod 	= "select * from sa90prod_so1 where so = '$so' and partno = '$part'";
	...
	if($cek == 0)          
	  {
		...
		echo $part;
		echo ' is nothing.. inserting ...<br />';
				$ins_sa90prod = "insert into sa90prod_so1 select * from sa90prodtemp_so1 where so = '$so' and partno = '$part'";
		...
		echo ' data already exist, updating ...<br />';
			...
			$upd_sa90prod = "update sa90prod_so1 set partname = '$partname', bom = $bom, qty = $qty, lot = '$lot', ";
...
$sqltampil = "select * from sa90_so1";
