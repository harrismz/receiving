...
****	on 6 Sep 2016
****	revise: menambahkan portal hurus A di awal dan di akhir barcode (line: 64 dan 88)
*/
...
else
{
	...
	$showserial1 = substr($varserial,0,4);
	$showserial2 = substr($varserial,4,4);
	$showserial3 = substr($varserial,8,4);
	$showserial4 = substr($varserial,12,4);
	$showserial5 = substr($varserial,16,6);
	$showserial = $showserial1 . '-' . $showserial2 . '-' . $showserial3 . '-' . $showserial4 . '-' . $showserial5 ;
	//	get karakter di awal dan di akhir
	$left	= substr($varserial, 0, 1);
	$right	= substr($varserial, -1);	
	...
	if(strlen($txtbarcode) <> 22){
		...
		//	cek karakter kanan dan kiri serial
		if($left <> $right){
			echo "<font size=6 color=red>Serial <b><u>$showserial</u></b> Pembacaan serial error, coba lagi !!!</font><br />";
			$suara = 'WrongParts2good.wav';
			echo "<embed src =\"$suara\" hidden=\"true\" autostart=\"true\"></embed>";
		...
		//	end of cek karakter kanan dan kiri serial
	}
	// batas copy paste
}  
// else if post
