...
<script>
	...
	function view_partcode(){
		$('#view_partlist').html(
			'<p align="center" class="loading"><img src="img/loading.gif" alt="loading" height="100" width="100"/></p>'
		);
		$.post('print_partcode_sch_viewtes.php', {issdate: $('[name=issue_date]').val(), issdept: $('[name=issue_dept]').val()},
		function(result){
			$('#view_partlist').html(result).show();
		});
		return false;
	}
	function reset_partcode(){
		$('#view_partlist').html(
			'<p align="center" class="loading"><img src="img/loading.gif" alt="loading" height="100" width="100"/></p>'
		);
		$.post('print_partcode_sch_viewtes.php', {issdate: $('[name=""]').val(), issdept: $('[name=""]').val()},
		function(result){
			$('#view_partlist').html(result).show();
		});
		return false;
...
<body>
	...
	<br>
	<form method="post" action="print_partcode_schtes.php">
		...
		<!--	<div class="form-group col-sm-12">
			...
			<div class="form-group col-sm-12">
				<button class="btn btn-success col-sm-4 css_button" type="reset" name="reset" id="reset" value="RESET" onclick="reset_partcode()">RESET</button>
				<button class="btn btn-success col-sm-4 css_button" type="button" name="view" id="view" value="VIEW" onclick="view_partcode()">VIEW</button>
				<button class="btn btn-success col-sm-4 css_button" type="submit" name="submit" id="submit" value="SUBMIT">PRINT</button>
			</div>
		</div>
	</form>
	<div align="center" id="view_partlist" class="table table-responsive col-sm-12">
