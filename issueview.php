...
echo '<form method="post" action="issueview.php">';
echo 'FILTER DATA BY : &nbsp;&nbsp;';
...
if($pilihan == 'SO')
   {
	$sqltampil = "select * from partiss where so = '$nomorso' order by issdate";		
   }

if($pilihan == 'PART')
   {
	$sqltampil = "select * from partiss where partno = '$nomorso' order by issdate";		
   }
   
if($pilihan == 'PO')
   {
	$sqltampil = "select * from partiss where po = '$nomorso' order by issdate";		
   }
