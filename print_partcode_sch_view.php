			...
			try{
				if($issue_date !="" && $issue_dept =="" && $issue_line =="" && $issue_model =="" && $issue_prodno =="" && $issue_lot =="" && $issue_qty ==""){
					$sql = "SELECT distinct(a.[kode]),a.[partdept],a.[line],a.[model],a.[prod_no]
							...
							--,(select min(isnull(b.STATUS_SCAN, 0)) as status from partlist b where b.kode = a.kode) as minsts
							--,(select max(isnull(c.STATUS_SCAN, 0)) as status from partlist c where c.kode = a.kode  and isnull(c.STATUS_SCAN, 0) <> 2) as maxsts
							FROM [partlist] a
			...
			<tbody>
				...
				<?php
						...
						while(!$rssch->EOF){
							...
							//	get min
							$getmin = $db->Execute("select min(isnull(STATUS_SCAN, 0)) as status from partlist where kode = '{$rssch->fields[0]}'");
							$minsts = $getmin->fields[0];
							$getmin->Close();
							
							//	get max
							$getmax = $db->Execute("select max(isnull(STATUS_SCAN, 0)) from partlist where kode = '{$rssch->fields[0]}' and isnull(STATUS_SCAN, 0) <> 2");
							$maxsts = $getmax->fields[0];
							$getmax->Close();
