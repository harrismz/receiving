        ...
        while(!$brsbaca->EOF)
          {
			...
			// cek so di sa90m
			$sqlada_so = "select distinct so, model from sa90m where so = '$cekso'"; //by zaki 20170421
            $rshasil_so = $db->Execute("select distinct so, model from sa90m where so = '$cekso'"); //by zaki 20170421
			...
			if($cekdata_so != 0 && $cekmodel_so != $cekmodel2){ //by zaki 20170421
				$sqlins_dup = "insert into sa90m_dup select so,partno,partname,bom,qty,lot,line,model from sa90 where so = '$cekso' and partno = '$cekpart'";
