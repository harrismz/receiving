			...
			*/
			...
			/*end modify by zaki*/
			{
				...
				if ($wrongpart == 1){
					...
					if ($cekpart == 1){
						if ($issueke != ""){
							echo'<br><br>Manual)'.	$sqlpartlist = "SELECT id_partlist, partdept, line, model, prod_no, lot, qty,
							...
							echo'<br><br>'.		$sqlsel = "select * from partiss where so = '$txtso' and partno = '$varpart' and partname = '$nilainama' and po = '$varpo' and bom = '$nilaibom' and reqqty = '$nilaireq' and scanqty = '$varqty' and lot = '$nilailot' and line = '$nilailine' and model = '$nilaimodel' and issdate = '$tglins' and nik = '$txtnik'";
									...
									if ($chk_inspartiss != 0){
							echo'<br><br>updmanual--'.			$sqlupd_partlist = "UPDATE partlist set tot_scan = '$tot_qtyscan', scan_nik = '$txtnik', scan_date = '$tglins'
																WHERE
																		(status_scan IS NULL)
																	AND (partdept = '$dept_part')
																	AND (date_issue = '$date_part') 
									...
									echo'<br><br>passmanual--'.	$sqlupd_partlist = "UPDATE partlist set tot_scan = '$tot_qtyscan', status_scan = '1', scan_nik = '$txtnik', scan_date = '$tglins'
																WHERE
																		(status_scan IS NULL)
																	AND (partdept = '$dept_part')
																	AND (date_issue = '$date_part') 
								...
								echo'*issue*'. $issueke_oto = $chkissue->fields['0'];
								
						
							echo'<br><br>auto)'.	$sqlpartlist = "SELECT id_partlist, partdept, line, model, prod_no, lot, qty,
											date_issue, so_number, no, zone, partno, partname, demand, issue_ke, std_issue,
											scan_issue, tot_scan,status_scan, upload_nik, upload_date, scan_nik, scan_date
											FROM partlist
											WHERE
							...
							echo'<br><br>'.		$sqlsel = "select * from partiss where so = '$txtso' and partno = '$varpart' and partname = '$nilainama' and po = '$varpo' and bom = '$nilaibom' and reqqty = '$nilaireq' and scanqty = '$varqty' and lot = '$nilailot' and line = '$nilailine' and model = '$nilaimodel' and issdate = '$tglins' and nik = '$txtnik'";
									...
									if ($chk_inspartiss != 0){
							echo'<br><br>updauto--'.			$sqlupd_partlist = "UPDATE partlist set tot_scan = '$tot_qtyscan', scan_nik = '$txtnik', scan_date = '$tglins'
																WHERE
																		(status_scan IS NULL)
																	AND (partdept = '$dept_part')
																	AND (date_issue = '$date_part') 
									...
									if ($chk_inspartiss2 != 0){
									echo'<br><br>passauto--'.	$sqlupd_partlist = "UPDATE partlist set tot_scan = '$tot_qtyscan', status_scan = '1', scan_nik = '$txtnik', scan_date = '$tglins'
																WHERE
																		(status_scan IS NULL)
																	AND (partdept = '$dept_part')
																	AND (date_issue = '$date_part') 
