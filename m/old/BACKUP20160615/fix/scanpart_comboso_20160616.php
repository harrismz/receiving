		...
		<?php
			...
			try{
				...
				*/
				/*20160615*
				if ($issueke == ""){
					$sql = "SELECT no, so_number, zone, partno, partname, demand, issue_ke, std_issue, scan_issue, tot_scan, status_scan,
				...
				*20160615*/
				if ($issueke == ""){
				echo	$sql = "SELECT b.no, b.so_number, b.zone, b.partno, b.partname, b.demand, b.issue_ke, b.std_issue, b.scan_issue, b.tot_scan, b.status_scan,
					(select sum(a.scanqty) from partiss a where a.so = b.so_number and a.partno = b.partno) as alltotscan
					FROM partlist b
						WHERE (b.partdept = '$dept_part') AND (b.date_issue = '$date_part') AND (b.line = '$line') AND (b.model = '$model') AND (b.prod_no = '$prodno') AND (b.lot = '$lot') AND (b.qty = '$qty')
						AND (b.issue_ke = (SELECT MIN(issue_ke) as issueke FROM partlist WHERE (status_scan IS NULL) AND (partdept = '$dept_part') AND (date_issue = '$date_part') AND (line = '$line') AND (model = '$model') AND (prod_no = '$prodno') AND (lot = '$lot') AND (qty = '$qty') and (scan_issue <> 0)))";
						$rspart = $db->Execute($sql);
					//	$exist = $rspart->RecordCount();
