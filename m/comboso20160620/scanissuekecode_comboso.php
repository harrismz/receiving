<?php
	...
	try{
		if ($issueke == ""){
			...
			$sql = "SELECT MIN(issue_ke) as issueke FROM partlist WHERE (status_scan IS NULL) AND (kode = '$partcode') AND (scan_issue <> 0)";
			$rspart = $db->Execute($sql);
			
			$sql2 = "SELECT MAX(issue_ke) as issueke FROM partlist WHERE (status_scan IS NULL) AND (kode = '$partcode') AND (scan_issue <> 0)";
