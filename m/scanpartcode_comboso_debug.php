		...
		<?php
			...
			try{
				if ($issueke == ""){
					...
					,(select top 1 sum(c.tot_scan) as grdtotal from partlist c where c.kode = b.kode and c.partno = b.partno) as grdtotal
					FROM partlist b
						WHERE (b.kode = '$partcode') 
						AND (b.issue_ke = ISNULL(
											(SELECT MIN(issue_ke) as issueke FROM partlist
											WHERE (status_scan) IS NULL
											AND (kode = '$partcode')
											AND (scan_issue <> 0)),
											(SELECT MAX(issue_ke) as issueke FROM partlist
											WHERE (kode = '$partcode')
											AND (scan_issue <> 0))
										))
						order by zone asc";
		...
		<table id="dt_partlist" align="center">
			...
			<tbody>
				...
				<?php
						...
						try{
							$sql = "UPDATE partlist SET prcode_scn_status = 1 where kode = '$partcode'";
							$rs_prcdscn  = $db->Execute($sql);
						}
						catch(Exception $e){
							echo '[[[SQL SERVER]]] :::'.$ex->getMessage();
						}
						...
						while(!$rspart->EOF){
							...
							elseif( intval($prt_scan_issue) != intval($prt_std_issue) and intval($prt_scan_issue) != '0'){
								echo'<tr class="ast-focus" style="background-color:red; color:white;">';
							}
							else{
								echo'<tr class="ast-focus">';
							}
							$alltot_scan   = number_format($rspart->fields['11'], 2, '.', '');
							$prt_grdtotal  = number_format($rspart->fields['12'], 2, '.', '');
				?>
								<td><?=$no?></td>
								<td><?=$prt_sonum?></td>
								<td><?=$prt_zone?></td>
