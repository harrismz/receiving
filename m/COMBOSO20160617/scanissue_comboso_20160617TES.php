			...
			/*end modify by zaki*/
			{
				...
				if ($wrongpart == 1){
					...
					if ($cekpart == 1){
						...
						echo	$sqlchkcode = "SELECT partdept, line, model, prod_no, lot, qty, date_issue FROM partlist
										WHERE kode = '{$partcode}'";
							$rschkcode = $db->execute($sqlchkcode);
							
						echo"<br>".	$dept_part = $rschkcode->fields[0];
						echo"<br>".	$line = $rschkcode->fields[1];
						echo"<br>".		$model = $rschkcode->fields[2];
						echo"<br>".		$prodno = $rschkcode->fields[3];
						echo"<br>".		$lot = $rschkcode->fields[4];
						echo"<br>".		$qty = $rschkcode->fields[5];
						echo"<br>".		$date_part = $rschkcode->fields[6];
							
						echo	$sqlpartlist = "SELECT id_partlist, partdept, line, model, prod_no, lot, qty,
