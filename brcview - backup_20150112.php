...
if ($_SERVER['REQUEST_METHOD'] != 'POST')
...
else
  {
	...
	echo '<br>';
	echo '<a href="http://136.198.117.40/barcode_sato/index.php?partno='. $partno .'&po='. $po .'&suppname='. $suppname .'&pack='. $pack .'&qtystd='. $qtystd .'&qtybal='. $qtybal .'&qty='. $qty .'&qty='. $qty .'&lokasi='. $lokasi .'&supp='. $supp .'&invno='. $invno .'">Print SATO Label</a></td>'; 
	
	$host		= getenv("REMOTE_ADDR");
	if(  $host == '10.230.30.104' ){
		echo '<br>';
