...
<html>
			...
			try{
				if($issue_date !="" && $issue_dept =="" && $issue_line =="" && $issue_model =="" && $issue_prodno =="" && $issue_lot =="" && $issue_qty ==""){
					...
					$sql = "SELECT distinct([kode]),[partdept],[line],[model],[prod_no]
							,[lot],[qty],[date_issue]
							FROM [partlist]
							WHERE [date_issue] = '{$issue_date}' and [partdept] = '{$issue_dept}'";
					$rssch = $db->Execute($sql);
					$exist = $rssch->RecordCount();
				}
				else{
					$sql = "SELECT distinct([kode]),[partdept],[line],[model],[prod_no]
							  ,[lot],[qty],[date_issue] FROM [partlist]
	...
	<!--	<h3 align="center">PARTLIST SCHEDULE <?/*=date_format(date_create($issue_date), 'd F Y')*/?></h3> -->
		<h3 align="center">PARTLIST SCHEDULE <?=$tgl_skrg?></h3>
		<table id="dt_printpartlist" border="1" style="border-collapse: collapse;">
			<thead>
				<tr>
					<th>NO</th>
					<th>DEPT</th>
