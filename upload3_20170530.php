        ...
        while(!$brsbaca->EOF)
          {
			...
			// cek so di sa90m
			$sqlada_so = "select distinct so, model from sa90m_so1 where so = '$cekso'"; //by zaki 20170421
			...
			if($cekdata_so != 0 && $cekmodel_so != $cekmodel2){ //by zaki 20170421
				$sqlins_dup = "insert into sa90m_dup select so,partno,partname,bom,qty,lot,line,model from sa90_so1 where so = '$cekso' and partno = '$cekpart'";
				...
				if($cekdata == 0){
						...
						echo ' is nothing.. inserting ...<br />';
							$sqlins = " insert into sa90m_so1 select so,partno,partname,bom,qty,lot,line,model from sa90 where so = '$cekso' and partno = '$cekpart'";
	...
	if($cek == 0)          
	  {
		...
		echo $part;
		echo ' is nothing.. inserting ...<br />';
				$ins_sa90prod = "insert into sa90prod_so1 select * from sa90prodtemp where so = '$so' and partno = '$part'";					
