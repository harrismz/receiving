...
<?php
	...
	if(isset($_GET['partno']))
	{
		...
		if( $qtystd != 0){
			...
			$data .= $esc . 'A';
			$data .= $esc . 'A3H1374V0001';
			$data .= $esc . 'H0155' . $esc . 'V0010' . $esc . 'L0202' . $esc . 'L0202' . $esc . 'S' . $invno . $esc . 'XU' . $barcode_a . $esc . 'L0202' . $esc . 'S' . $lokasi;
			$data .= $esc . 'H0155' . $esc . 'V0038' . $esc . 'B101050*' . $barcode_b . '*';
			$data .= $esc . 'H0160' . $esc . 'V0093' . $esc . 'L0202' . $esc . 'S' . $partno . $esc . 'L0203' . $esc . 'XU' . $barcode_c;
			$data .= $esc . 'Q'. $qtystd .'';
			$data .= $esc . 'Z';
		...
		$dir 	= "\\\\$compname\\PrintSato\\";
		
		$myfile 	= fopen($dir . "print_". $Ymd . $wkt .".txt", "w") or die("Unable to open file!");
		$txt 		= $print;
		fwrite($myfile, $txt);
		fclose($myfile);
